...
class Custom(data.Dataset):
    def __init__(self, data_path, imageset='infer', return_ref=False):
        ...
        self.color_map_inv = semkittiyaml['color_map_inv']
        self.label_map = semkittiyaml['labels']
        self.imageset = imageset
        split = semkittiyaml['split']['infer']

        self.im_idx = []
        for i_folder in split:
            self.im_idx += absoluteFilePaths('/'.join([data_path, str(i_folder).zfill(2), 'pointcloud']))
        self.im_idx.sort()
    ...
    def __getitem__(self, index):
        if type(self.im_idx[index]) is np.ndarray:
            raw_data = self.im_idx[index]

        elif self.im_idx[index].endswith('.ply'):
            raw_data = PlyData.read(self.im_idx[index])
            x = raw_data['vertex']['x']
            y = raw_data['vertex']['y']
            z = raw_data['vertex']['z']
            i = raw_data['vertex']['intensity']
            raw_data = np.stack([x, y, z, i]).T

        elif self.im_idx[index].endswith('.txt'):
            raw_data = np.loadtxt(self.im_idx[index],
                                  usecols=(0, 1, 2, 3),
                                  max_rows=124605,
                                  dtype=np.float32)

        elif self.im_idx[index].endswith('.bin'):
            raw_data = np.fromfile(self.im_idx[index], dtype=np.float32).reshape((-1, 4))

        else:
            print(index, " is not a valid file type")
